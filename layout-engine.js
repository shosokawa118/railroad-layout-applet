// =============================================================
// 鉄道模型レイアウトジェネレータ - 基本エンジン (自動サイズ計算版)
// バージョン: VER-AUTO-SIZE-P6
// =============================================================
console.log("基本エンジン（JS）が読み込まれました: VER-AUTO-SIZE-P6");

let globalJoints = [];
let railCount = 0;
let isFirstMoveFrame = true;

// カタログの幾何学パラメータからバラストポリゴンパスを自動計算
...
function addRailToCanvas(partId) {
    ...
    // ★width, heightをあえて指定しないことで、Fabric.jsに「正確な実寸サイズ」を100%自動計算させ、グラフィックの歪みを追放
    let railObject = new fabric.Path(autoGeneratedPath, {
        fill: '#888888'
    });

    // ★原点を「完全な中心(center)」に統一。自動計算された枠の中心が自動的に (0,0) になります
    railObject.set({
