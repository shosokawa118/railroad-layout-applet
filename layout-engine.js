// =============================================================
// 鉄道模型レイアウトジェネレータ - 基本エンジン (8px描画ズレ完全解消版)
// バージョン: VER-PERFECT-RENDER-R9
// =============================================================
console.log("基本エンジン（JS）が読み込まれました: VER-PERFECT-RENDER-R9");
...
function addRailToCanvas(partId) {
    ...
    // ★【修正：真の幾何学中心の自動抽出ロジック】
    // 枠線の太さ(stroke)などのグラフィックのゴミを完全に排除するため、
    // パスオブジェクトの内部に格納されている純粋な数式上の境界データ（pathBounds）をダイレクトに読み取ります。
    const tempPathForCalc = new fabric.Path(autoGeneratedPath);
    const pBounds = tempPathForCalc.pathBounds; // 線の太さを無視した純粋な数式上の[左右上下]の限界値
    
    // 数式上の純粋なポリゴンの中心XYを割り出し
    const fabricCenterX = pBounds.left + pBounds.width / 2;
    const fabricCenterY = pBounds.top + pBounds.height / 2;

    // 本物のレールオブジェクトを生成
    let railObject = new fabric.Path(autoGeneratedPath, {
        fill: '#888888',
        // 抽出したピュアな幾何学のズレを pathOffset に指定することで、見た目の8pxズレを100%相殺します。
...
function loadDebugSampleLayout() {
    ...
    updateJointIndicators();
    canvas.requestRenderAll();
    console.log("[%s] サンプル小判型エンドレスを自動サイズ計算で100%%復元しました！", CODE_VERSION);
}
