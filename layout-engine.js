// =============================================================
// 鉄道模型レイアウトジェネレータ - 基本エンジン (自動オフセット相殺版)
// バージョン: VER-GENERIC-OFFSET-Q8
// =============================================================
console.log("基本エンジン（JS）が読み込まれました: VER-GENERIC-OFFSET-Q8");

let globalJoints = [];
let railCount = 0;
let isFirstMoveFrame = true;

// 形状パラメータからバラストポリゴンパスを自動計算
function generateGenericRailPath(catalogItem) {
    let combinedSvgPath = "";
    const BALLAST_WIDTH = 16;
    const halfW = BALLAST_WIDTH / 2;

    if (!catalogItem || !catalogItem.shapes) return "M 0 0 L 10 0";
...
function addRailToCanvas(partId) {
    ...
    // ★【自動ズレ量計算ロジック】
    // パスを仮生成し、Fabricが自動計算したバウンディングボックスの「物理的中心(BoundingCenter)」と、
    // カタログ側の幾何学的な「数式原点(0,0)」の間のズレ（オフセット）をミリ単位で自動抽出します。
    const tempPathForCalc = new fabric.Path(autoGeneratedPath);
    const bounds = tempPathForCalc.getBoundingRect();
    
    // Fabricが算出した、(0,0)から見た物理的枠の中心点
    const fabricCenterX = bounds.left + bounds.width / 2;
    const fabricCenterY = bounds.top + bounds.height / 2;

    // 本物のレールオブジェクトを生成
    let railObject = new fabric.Path(autoGeneratedPath, {
        fill: '#888888',
        // ★抽出したズレ(fabricCenter)を pathOffset にそっくりそのまま指定することで、
        // どんな非対称パーツであっても自動的に軸ズレが100%相殺（クレンジング）され、数式上の (0,0) が完璧な原点になります。
        pathOffset: new fabric.Point(fabricCenterX, fabricCenterY)
    });
...
function loadDebugSampleLayout() {
    ...
    updateJointIndicators();
    canvas.requestRenderAll();
    console.log("[%s] サンプル小判型エンドレスを自動相殺演算で100%%復元しました！", CODE_VERSION);
}
